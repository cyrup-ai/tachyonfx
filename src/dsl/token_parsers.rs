...
use anpa::combinators::{
    and_parsed, attempt, many_to_vec, middle, no_separator, separator, succeed,
};
...
fn sequence<'a>() -> impl TokenParser<'a, Expr> {
    ...
    .map(|(span, (_, _, args, self_fns))| Expr::Sequence {
        effects: args,
        self_fns,
        span,
    })
...
fn parallel<'a>() -> impl TokenParser<'a, Expr> {
    ...
    .map(|(span, (_, _, args, self_fns))| Expr::Parallel {
        effects: args,
        self_fns,
        span,
    })
...
fn sanitize_syntax(args: Vec<Expr>) -> Vec<Expr> {
    let (args, delimeters): (Vec<_>, Vec<_>) =
        args.into_iter().enumerate().partition(|(i, _)| i & 1 == 0);
    ...
    impl Expr {
        ...
        fn with_span(self, start: u32, end: u32) -> Self {
            ...
            match self {
                ...
                Var { name, self_fns, .. } => Var {
                    name,
                    self_fns,
                    span,
                },
                LetBinding { name, let_expr, .. } => LetBinding {
                    name,
                    let_expr,
                    span,
                },
        ...
        with_tokens("Color::Red, 500, fx::dissolve(200)", |tokens| {
            assert_eq!(
                ...
                Some(vec![
                    ...
                    expr_fn_call(
                        "fx::dissolve",
                        vec![Expr::Literal(Value::U32(200), ExprSpan::new(30, 33))]
                    )
                    .with_span(17, 34)
                ])
            );
                    ...
                    Some(
                        fn_info(
                            ...
                            vec![
                                expr_fn_call(
                                    "fx::dissolve",
                                    vec![Expr::Literal(Value::U32(200), ExprSpan::new(26, 29))]
                                )
                                .with_span(13, 30),
                                expr_fn_call(
                                    "fx::fade_to",
            ...
            |tokens| {
                assert_eq!(
                    ...
                    Some(
                        expr_fn_call(
                            "fx::dissolve",
                            vec![Expr::Literal(Value::U32(200), ExprSpan::new(13, 16))]
                        )
                        .with_self_fns(vec![
                            fn_info(
                                "filter",
                ...
                assert_eq!(
                    ...
                    Some(Expr::Array(
                        vec![
                            expr_fn_call(
                                "fx::dissolve",
                                vec![Expr::Literal(Value::U32(200), ExprSpan::new(14, 17))]
                            )
                            .with_span(1, 18),
                            expr_fn_call(
                                "fx::fade_to",
                ...
                assert_eq!(
                    ...
                    Some(Expr::ArrayRef(
                        vec![
                            expr_fn_call(
                                "fx::dissolve",
                                vec![Expr::Literal(Value::U32(200), ExprSpan::new(15, 18))]
                            )
                            .with_span(2, 19),
                            expr_fn_call(
                                "fx::fade_to",
            ...
            assert_eq!(
                ...
                Some(Expr::OptionSome(
                    Box::new(
                        expr_fn_call(
                            "fx::dissolve",
                            vec![Expr::Literal(Value::U32(200), ExprSpan::new(18, 21))]
                        )
                        .with_span(5, 22)
                    ),
                    ExprSpan::new(0, 23)
    ...
    fn test_sequence_parser() {
        ...
        with_tokens("fx::sequence(&[fx::dissolve(200)])", |tokens| {
            assert_eq!(
                parse(sequence(), tokens).result,
                Some(Expr::Sequence {
                    effects: vec![expr_fn_call(
                        "fx::dissolve",
                        vec![Expr::Literal(Value::U32(200), ExprSpan::new(28, 31))]
                    )
                    .with_span(15, 32)],
                    self_fns: vec![],
                ...
                assert_eq!(
                    ...
                    Some(Expr::Sequence {
                        effects: vec![
                            expr_fn_call(
                                "dissolve",
                                vec![Expr::Literal(Value::U32(200), ExprSpan::new(24, 27))]
                            )
                            .with_span(15, 28),
                            expr_fn_call(
                                "fx::fade_to",
