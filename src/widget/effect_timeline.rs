...
impl EffectTimeline {
    ...
    fn render_timeline_intervals(&self, root: &EffectSpan, chart_row: Rect, buf: &mut Buffer) {
        ...
        Span::from(last_label).style(style).render(area, buf);
    ...
    fn render_cell_filter_legend(&self, area: Rect, buf: &mut Buffer) {
        ...
        self.cell_filter_resolver
            ...
            .for_each(|((id, filter), row)| {
                ...
                row.x += 6;
                Span::from(filter).style(legend_style).render(row, buf);
            });
    }
    ...
    pub fn layout(&self, area: Rect) -> EffectTimelineRects {
        ...
        let areas_col_w = if self.area_resolver.entries().is_empty() {
            0
        } else {
            4
        };
...
impl Widget for EffectTimeline {
        ...
        Self: Sized,
    {
        ...
        // cell filter column
        let filters: Vec<_> = self.span.iter().map(|span| &span.cell_filter).collect();
...
mod tests {
    ...
    fn test_widget_happy_path() {
        let bg = Color::Black;
        let fx = sequence(&[
            fx::sweep_out(Motion::DownToUp, 5, 0, bg, (2000, QuadOut)),
            fx::sweep_in(Motion::UpToDown, 5, 0, bg, (2000, QuadOut)),
            fx::sweep_out(Motion::UpToDown, 5, 0, bg, (2000, QuadOut)),
            fx::sweep_in(Motion::DownToUp, 5, 0, bg, (2000, QuadOut)),
        ]);
